...
main プロセスは, 子プロセスの管理, ユーザー設定やアプリのアップデートなどを担当するのみ. 規模は小さい.
renderer プロセスと extensionHost のコードの規模が大きい.

独自実装の DI コンテナが使われている. いろいろなところで使われているので, DI コンテナを理解していないとコードを読むのが大変.
依存するサービスが静的に決定される, プロセス内でユニークなオブジェクトは, DI コンテナが生成のすべてを管理する.
registerSingleton を呼んでクラスの id を登録する. DI コンテナの起動は、
...
RPC経由で呼ぶメソッドの名前には先頭に$をつける規則になっている.
例えば, 以下の $createEditorInset はコードは renderer プロセスで実行されるが,
RPCProtocol 経由で extensionHost から呼び出される.
